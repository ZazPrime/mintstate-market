-- Character equity: decomposes a card's price into the drivers behind it —
-- what it costs to pull, what the character premium adds, what grading adds,
-- and how fast the card actually trades.

-- Strips grade/mechanic suffixes so "Charizard ex" and "Charizard VMAX" roll
-- up to the same character.
create or replace function public.character_name(card_name text)
returns text language sql immutable as $$
  select initcap(trim(regexp_replace(
    regexp_replace(lower(card_name),
      '\s*(ex|gx|v|vmax|vstar|v-union|break|prime|lv\.x|star|δ|◇)$', '', 'i'),
    '^(dark|light|shining|radiant|shadow|team [a-z]+''s|[a-z]+''s)\s+', '', 'i')));
$$;
...
select
  ...
  round((coalesce(b.market_price_psa10, 0) * coalesce(b.gem_rate, 0))::numeric, 2) as gem_adjusted_value,
